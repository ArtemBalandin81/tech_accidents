...
async def test_super_user_delete_file_id_url(
    ...
    scenarios - тестовые сценарии использования эндпоинта (сценарии НЕ СВЯЗАНЫ!!!).
    expected - словарь ожидаемых значений в сценарии.
    match_values - кортеж параметров, используемых в assert (ожидание - реальность).
    """
    ...
    scenarios = (
        # params, status, expected_key, name
        ({SEARCH_FILES_BY_ID: []}, 404, "no_files", "empty choice not found"),  # 1
        ({SEARCH_FILES_BY_NAME: "???", SEARCH_FILES_BY_ID: [1]}, 403, "no_files", "by name & ids simultaneously"),  # 2
        ({SEARCH_FILES_BY_NAME: "no exist"}, 404, "no_files", "search files by name no exist"),  # 3

        ({SEARCH_FILES_BY_ID: 1}, 200, "testfiles_3_2", "delete testfile"),  # 2
        ({SEARCH_FILES_BY_NAME: "testfile2"}, 200, "testfile3", "delete testfile2"),  # 2
        ({SEARCH_FILES_BY_NAME: "testfile"}, 200, "no_files", "delete files, found by part name"),  # 2
        # ({CHOICE_FORMAT: json_choice}, 404, "no_files", 55, "search files by id == 55 is not found"),  # 3
        # ({CHOICE_FORMAT: json_choice}, 200, "testfile2", 2, "testfile2 in json"),  # 4
        # ({CHOICE_FORMAT: json_choice}, 200, "testfile3", 3, "testfile3 in json"),  # 5
        # ({CHOICE_FORMAT: files_choice}, 200, "testfile3", 3, "download testfile3"),  # 6
    ...
    # - созданы таски todo создать suspension
    # - с помощью async_db прикрепить файлы к таске и простою  todo
    # - сделать проверки, что прикрепились и есть  todo
    # - удалить файл, не прикрепленный  todo
    # - удалить файл прикрепленный к таске и простою  todo
    # - к одному из них  todo
    ...
    async with async_client as ac:
        ...
        # !!!! starting testing scenarios:
        for params, status, expected_key, name in scenarios:  # todo file_id - лишнее
            ...
            await log.awarning(
                ...
                params=params,
                response=response.json(),
                # response_files=[_[FILE_NAME] for _ in response.json().get(FILES_UNUSED_IN_DB_REMOVED)],
                # response=response.json() if params.get(CHOICE_FORMAT) == json_choice else None,
                # uploaded_files=uploaded_files,
                # login_data=user_orm_login,
                # response=response.__dict__,
        ...
        await clean_test_database(async_db, FileAttached, Task, TasksFiles)  # clean db after test todo suspensions
        await delete_files_in_folder(file_paths)
        all_files_in_folder = [file.name for file in FILES_DIR.glob('*')]
        if uploaded_files is not None:
            for file in uploaded_files:
