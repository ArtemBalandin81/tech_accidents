...
@test_router.get(GET_TEST_URL, description="Проверка доступа к сайту.")
def test_get_url(
...
async def db_backup(
    ...
    if total_backups == 0:
        log.aerror("{}".format(DIR_CREATED_ERROR), list_of_files=list_of_files_names, folder="/db_backup")
