...
from src.api.constants import CONNECTION_TEST_URL_AGI, CONNECTION_TEST_URL_YA, TZINFO
...
class ConnectionErrorService:
    ...
    async def run_create_suspension(self, suspension_object: dict | None) -> None:
        ...
        async with self._sessionmaker() as session:
            ...
            print(f"Сохранен случай простоя в БД: {suspension}")  # TODO логгирование
        ...
        try:
            while True:
                ...
                if time_counter != settings.SLEEP_TEST_CONNECTION:  # Нач. счетчик простоя = интервалу теста соединения
                    print(f"suspension_start: {suspension_start}")  # TODO логгирование
            ...
            time_counter += settings.SLEEP_TEST_CONNECTION
            print(f"1st_time_counter: {time_counter} / suspension_START: {suspension_start}")  # TODO логгирование
            await asyncio.sleep(settings.SLEEP_TEST_CONNECTION)
